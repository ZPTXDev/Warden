...
export default {
    data: new SlashCommandBuilder()
        .setName('say')
        .setDescription(
            getLocaleString(settings.defaultLocaleCode, 'CMD.SAY.DESCRIPTION'),
        )
        .addStringOption(
    ...
    async execute(
        ...
        let player = interaction.client.tts.players.get(
            interaction.guildId,
        ) as WardenPlayer;
        if (player?.playing) {
            await interaction.replyHandler.locale(
                'CMD.SAY.RESPONSE.IN_PROGRESS',
                { type: 'error' },
            );
            return;
        }
        await interaction.deferReply();
        let prepend = await data.guild.get(
            interaction.guildId,
            'settings.prepend',
        );
        switch (prepend) {
        ...
        for (const url of urls) {
            const result = await interaction.client.tts.rest.loadTracks(
                url.url,
            );
        ...
        if (errored) {
            await interaction.replyHandler.locale(
                'CMD.SAY.RESPONSE.LOAD_FAIL',
                { type: 'error' },
            );
            return;
        }
        if (!player?.connected) {
            player = interaction.client.tts.createPlayer(
                interaction.guildId,
            ) as WardenPlayer;
        ...
        await interaction.replyHandler.reply(
            new EmbedBuilder().setDescription(rawMessage).setAuthor({
