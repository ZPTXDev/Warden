...
export default {
	data: new SlashCommandBuilder()
		.setName('cancel')
		.setDescription(getLocaleString(settings.defaultLocaleCode, 'CMD.CANCEL.DESCRIPTION')),
	...
	async execute(interaction: WardenInteraction<ChatInputCommandInteraction>): Promise<void> {
		...
		if (!player.queue.current || !player.playing && !player.paused) {
			await interaction.replyHandler.locale('TTS.PLAYER.PLAYING.NOTHING', { type: 'error' });
			return;
		}
		player.queue.clear();
