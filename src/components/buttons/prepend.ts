...
export default {
	name: 'prepend',
	async execute(interaction: WardenInteraction<ButtonInteraction>): Promise<void> {
		...
		const { current, embeds, actionRow } = await buildSettingsPage(interaction, guildLocaleCode, 'prepend');
		const description = `${getLocaleString(guildLocaleCode, 'CMD.SETTINGS.RESPONSE.HEADER', interaction.guild.name)}\n\n**${getLocaleString(guildLocaleCode, 'CMD.SETTINGS.MISC.PREPEND.NAME')}** ─ ${getLocaleString(guildLocaleCode, 'CMD.SETTINGS.MISC.PREPEND.DESCRIPTION')}\n> ${getLocaleString(guildLocaleCode, 'MISC.CURRENT')}: \`${current}\``;
