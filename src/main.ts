import type { Button, ChatInputCommand, ModalSubmit, SelectMenu } from '#src/events/interactionCreate.d.js';
...
import { Client, Collection, EmbedBuilder, GatewayDispatchEvents, GatewayIntentBits } from 'discord.js';
...
export const bot: WardenClient = new Client({ intents: [GatewayIntentBits.Guilds, GatewayIntentBits.GuildVoiceStates] });
...
bot.ws.on(GatewayDispatchEvents.VoiceServerUpdate, async (payload): Promise<void> => bot.tts.handleVoiceUpdate(payload));
bot.ws.on(GatewayDispatchEvents.VoiceStateUpdate, async (payload): Promise<void> => bot.tts.handleVoiceUpdate(payload));
...
export async function shuttingDown(eventType: string, err?: Error): Promise<void> {
	...
	inProgress = true;
	logger.info({ message: `Shutting down${eventType ? ` due to ${eventType}` : ''}...`, label: 'Warden' });
	try {
		if (startup.started) {
			...
			for (const pair of players) {
				...
				const success = await player.handler.send(
					new EmbedBuilder()
						.setDescription(`${await getGuildLocaleString(player.guildId, ['exit', 'SIGINT', 'SIGTERM', 'lavalink'].includes(eventType) ? 'TTS.PLAYER.RESTARTING.DEFAULT' : 'TTS.PLAYER.RESTARTING.CRASHED')}`)
						.setFooter({ text: await getGuildLocaleString(player.guildId, 'TTS.PLAYER.RESTARTING.APOLOGY') }),
					{ type: 'warning' },
				);
				if (!success) continue;
			}
		...
		if (error instanceof Error) {
			logger.error({ message: 'Encountered error while shutting down.', label: 'Warden' });
...
for await (const file of eventFiles) {
	const event: { default: WardenEvent } = await import(getAbsoluteFileURL(import.meta.url, ['events', file]).toString());
...
const ttsEventFiles = readdirSync(getAbsoluteFileURL(import.meta.url, ['events', 'tts'])).filter((file): boolean => file.endsWith('.js') || file.endsWith('.ts'));
for await (const file of ttsEventFiles) {
	const event: { default: WardenTTSEvent } = await import(getAbsoluteFileURL(import.meta.url, ['events', 'tts', file]).toString());
	if (event.default.once) {
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		bot.tts.once(event.default.name as keyof NodeEvents, (...args: any[]): void | Promise<void> => event.default.execute(...args));
	}
	else {
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		bot.tts.on(event.default.name as keyof NodeEvents, (...args: any[]): void | Promise<void> => event.default.execute(...args));
	}
}
