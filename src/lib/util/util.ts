...
import type { APISelectMenuOption, GuildMember, Interaction, InteractionReplyOptions, MessageActionRowComponentBuilder, MessageCreateOptions, Snowflake } from 'discord.js';
import { ActionRowBuilder, ButtonBuilder, ButtonStyle, EmbedBuilder, SelectMenuBuilder } from 'discord.js';
...
export async function buildSettingsPage(interaction: Interaction, guildLocaleCode: string, option: SettingsPageOptions): Promise<SettingsPage> {
	...
	switch (option) {
		case 'language':
			...
			break;
		case 'prepend': {
			current = await data.guild.get<string>(interaction.guildId, 'settings.prepend') ?? 'none';
			actionRow.addComponents(
				new ButtonBuilder()
					.setCustomId('prepend_none')
					.setLabel(getLocaleString(guildLocaleCode, 'CMD.SETTINGS.MISC.PREPEND.OPTIONS.NONE'))
					.setStyle(current === 'none' ? ButtonStyle.Success : ButtonStyle.Secondary)
					.setDisabled(current === 'none'),
				new ButtonBuilder()
					.setCustomId('prepend_username')
					.setLabel(getLocaleString(guildLocaleCode, 'CMD.SETTINGS.MISC.PREPEND.OPTIONS.USERNAME'))
					.setStyle(current === 'username' ? ButtonStyle.Success : ButtonStyle.Secondary)
					.setDisabled(current === 'username'),
				new ButtonBuilder()
					.setCustomId('prepend_nickname')
					.setLabel(getLocaleString(guildLocaleCode, 'CMD.SETTINGS.MISC.PREPEND.OPTIONS.NICKNAME'))
					.setStyle(current === 'nickname' ? ButtonStyle.Success : ButtonStyle.Secondary)
					.setDisabled(current === 'nickname'),
			);
			let example = '';
			switch (current) {
				case 'none':
					example = getLocaleString(guildLocaleCode, 'CMD.SETTINGS.MISC.PREPEND.EXAMPLE.NONE');
					break;
				case 'username':
					example = getLocaleString(guildLocaleCode, 'CMD.SETTINGS.MISC.PREPEND.EXAMPLE.ENABLED', interaction.user.username);
					break;
				case 'nickname':
					example = getLocaleString(guildLocaleCode, 'CMD.SETTINGS.MISC.PREPEND.EXAMPLE.ENABLED', (interaction.member as GuildMember)?.nickname ?? interaction.user.username);
					break;
			}
			embeds = [
				new EmbedBuilder()
					.setDescription(`${getLocaleString(guildLocaleCode, 'CMD.SETTINGS.MISC.PREPEND.EXAMPLE_DESCRIPTION')}\n> ${example}`)
					.setColor(settings.colors.neutral),
			];
			current = getLocaleString(guildLocaleCode, `CMD.SETTINGS.MISC.PREPEND.OPTIONS.${current.toUpperCase()}`);
		}
	}
