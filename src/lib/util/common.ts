...
export const logger = createLogger({
	...
	format: format.combine(
		...
		format.timestamp(),
		format.printf((info): string => `${info.timestamp} [${info.label}] ${info.level.toUpperCase()}: ${info.message}`),
	),
	transports: [
		new transports.Console({
			format: format.combine(
				...
				format.timestamp(),
				format.colorize(),
				format.printf((info): string => `${info.timestamp} [${info.label}] ${info.level}: ${info.message}`),
			),
		}),
