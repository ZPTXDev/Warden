...
export default {
	...
	async execute(oldState: VoiceState, newState: VoiceState): Promise<void> {
		...
		if (oldState.member.user.id === oldState.client.user.id) {
			...
			if ((oldState.serverMute !== newState.serverMute || oldState.serverDeaf !== newState.serverDeaf) && oldState.channelId === newState.channelId) return;
			/** Checks for when Warden leaves */
			// Disconnected
			if (!newState.channelId) {
				logger.info({ message: `[G ${player.guildId}] Cleaning up`, label: 'Warden' });
				player.channelId = null;
				await player.handler.locale('MUSIC.SESSION_ENDED.FORCED.DISCONNECTED', { type: 'warning' });
			...
			if (newState.channel.type === ChannelType.GuildVoice) {
				...
				if (!permissions.has(new PermissionsBitField([PermissionsBitField.Flags.ViewChannel, PermissionsBitField.Flags.Connect, PermissionsBitField.Flags.Speak]))) {
					await player.handler.locale('DISCORD.INSUFFICIENT_PERMISSIONS.BOT.BASIC', { type: 'error' });
					return player.handler.disconnect();
				}
			}
			if (newState.channel.type === ChannelType.GuildStageVoice) {
				await player.handler.locale('TTS.DISCONNECT.CHANNEL_UNSUPPORTED', { type: 'error' });
				return player.handler.disconnect();
			}
			// Moved to a new channel that has no humans
			if (newState.channel.members.filter((m): boolean => !m.user.bot).size < 1) {
				logger.info({ message: `[G ${player.guildId}] Disconnecting (alone)`, label: 'Warden' });
				await player.handler.locale('TTS.DISCONNECT.ALONE.DISCONNECTED.MOVED', { type: 'warning' });
				return player.handler.disconnect();
			}
		}
		...
		if (oldState.channel.members.filter((m): boolean => !m.user.bot).size >= 1) return;
		logger.info({ message: `[G ${player.guildId}] Disconnecting (alone)`, label: 'Warden' });
		await player.handler.locale('TTS.DISCONNECT.ALONE.DISCONNECTED.DEFAULT', { type: 'warning' });
		return player.handler.disconnect();
	},
};
