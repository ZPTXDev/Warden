...
export default {
	...
	once: false,
	async execute(interaction: WardenInteraction<Interaction>): Promise<void> {
		...
		if (interaction.isChatInputCommand()) {
			...
			if (failedPermissions.user.length > 0) {
				...
				await interaction.replyHandler.locale('DISCORD.INSUFFICIENT_PERMISSIONS.USER', { vars: [failedPermissions.user.map((perm): string => `\`${perm}\``).join(' ')], type: 'error' });
				return;
			}
			if (failedPermissions.bot.length > 0) {
				...
				await interaction.replyHandler.locale('DISCORD.INSUFFICIENT_PERMISSIONS.BOT.DEFAULT', { vars: [failedPermissions.bot.map((perm): string => `\`${perm}\``).join(' ')], type: 'error' });
				return;
			}
			try {
		...
		if (interaction.isButton()) {
			...
			if (!button) return;
			logger.info({ message: `[${interaction.guildId ? `G ${interaction.guildId} | ` : ''}U ${interaction.user.id}] Processing button ${interaction.customId}`, label: 'Warden' });
			try {
				logger.info({ message: `[${interaction.guildId ? `G ${interaction.guildId} | ` : ''}U ${interaction.user.id}] Executing button ${interaction.customId}`, label: 'Warden' });
				return button.execute(interaction);
			}
			catch (error) {
				if (error instanceof Error) {
					logger.error({ message: `[${interaction.guildId ? `G ${interaction.guildId} | ` : ''}U ${interaction.user.id}] Encountered error with button ${interaction.customId}`, label: 'Warden' });
					logger.error({ message: `${error.message}\n${error.stack}`, label: 'Warden' });
					await interaction.replyHandler.locale('DISCORD.GENERIC_ERROR', { type: 'error' });
				}
				return;
		...
		if (interaction.isSelectMenu()) {
			...
			logger.info({ message: `[${interaction.guildId ? `G ${interaction.guildId} | ` : ''}U ${interaction.user.id}] Processing select menu ${interaction.customId}`, label: 'Warden' });
			try {
				logger.info({ message: `[${interaction.guildId ? `G ${interaction.guildId} | ` : ''}U ${interaction.user.id}] Executing select menu ${interaction.customId}`, label: 'Warden' });
				return selectmenu.execute(interaction);
			}
			catch (error) {
				if (error instanceof Error) {
					logger.error({ message: `[${interaction.guildId ? `G ${interaction.guildId} | ` : ''}U ${interaction.user.id}] Encountered error with select menu ${interaction.customId}`, label: 'Warden' });
					logger.error({ message: `${error.message}\n${error.stack}`, label: 'Warden' });
					await interaction.replyHandler.locale('DISCORD.GENERIC_ERROR', { type: 'error' });
				}
				return;
		...
		if (interaction.isModalSubmit()) {
			...
			if (!modal) return;
			logger.info({ message: `[${interaction.guildId ? `G ${interaction.guildId} | ` : ''}U ${interaction.user.id}] Processing modal ${interaction.customId}`, label: 'Warden' });
			try {
				logger.info({ message: `[${interaction.guildId ? `G ${interaction.guildId} | ` : ''}U ${interaction.user.id}] Executing modal ${interaction.customId}`, label: 'Warden' });
				return modal.execute(interaction);
			}
			catch (error) {
				if (error instanceof Error) {
					logger.error({ message: `[${interaction.guildId ? `G ${interaction.guildId} | ` : ''}U ${interaction.user.id}] Encountered error with modal ${interaction.customId}`, label: 'Warden' });
					logger.error({ message: `${error.message}\n${error.stack}`, label: 'Warden' });
					await interaction.replyHandler.locale('DISCORD.GENERIC_ERROR', { type: 'error' });
				}
				return;
